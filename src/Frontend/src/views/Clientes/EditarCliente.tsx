...
export default function EditarCliente() {
  ...
  useEffect(() => {
    async function loadClient() {
      ...
      try {
        ...
        if (isNaN(parsedId)) {
          toast.error('El ID del cliente es inválido.');
        ...
        if (response.success && response.data) {
          ...
          toast.error(response.message || 'No se pudieron obtener los detalles del cliente.');
          navigate('/admin/clientes');
        }
      } catch (err) {
        console.error('Error al precargar cliente:', err);
  ...
  const handleSave = async (e: FormEvent) => {
    ...
    try {
      ...
      const response = await ClientesService.editarCliente(parseInt(id, 10), dto);
      if (response.success) {
        toast.success(response.message || 'Ficha de cliente actualizada.');
      ...
      if (data && !data.success && data.data && Array.isArray(data.data)) {
        ...
        toast.error(data?.message || 'Error de red al guardar los cambios.');
      }
    } finally {
      setSaving(false);
    }
    ...
    >
      ...
      <div className="mb-6">
        ...
        >
          ...
          Volver a ficha de {clientName}
        </button>
        <h2 className="font-display-lg text-display-lg text-ink mb-2">Editar Cliente</h2>
        <p className="text-body-md text-body-muted max-w-2xl">
          Modifique los datos de contacto y la información administrativa del cliente.
        </p>
      </div>
